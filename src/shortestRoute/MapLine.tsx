...
const MapLine = ({ imageRef, id1, id2 }: { imageRef: React.RefObject<HTMLImageElement>; id1: string; id2: string; }): JSX.Element => {
    ...
    return <>
        {(data1 && data2) ? <Line
            ...
            points={[
                ...
                data2.posx * ((imageRef.current?.width || 0) / totalWidth), 
                data2.posy * ((imageRef.current?.height || 0) / totalHeight)
            ]} stroke="Black" strokeWidth={16 * ((imageRef.current?.width || 0) / totalWidth)}
        /> : null}
    </>;
};
